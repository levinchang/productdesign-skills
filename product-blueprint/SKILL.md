...
用于在产品设计早期或版本规划时，输出一份结构化的「产品蓝图与场景说明」文档。该文档是 Feature List、Spec、PRD 与原型的**上游信息源**，统一描述产品定位、市场洞察、目标用户、关键场景与边界决策，兼容从 0-1 与 1-n 两种场景。
...
1. **版本信息与范围**
   ...
   - 可选：本轮先做项（`目标项ID（feature_id 或 module_id）` 或模块名），用于下游直接进入单项闭环。
2. **产品定位与价值主张**
   - 目标市场、替代方案、差异化主张。
3. **目标用户与核心场景**
   - 主角色、关键任务、触发频次/痛点强度。
4. **市场洞察与机会判断**
   - 竞品格局、机会窗口、进入理由（含证据来源）。
5. **商业目标与成功指标**
   - 北极星与阶段性目标（可量化或可验证）。
6. **关键场景列表**
...
9. **关键约束**
   - 合规、资源、组织、交付周期、技术前提等上位约束。
10. **核心风险与验证假设**
   - 最高风险假设、验证路径与判定标准。
11. **阶段策略（MVP 与里程碑）**
   - MVP 定义、阶段切分、Go/No-Go 条件。
12. **冻结边界（进入原型前）**
